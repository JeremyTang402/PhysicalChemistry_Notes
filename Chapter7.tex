...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item 定位系统 localized system：又称定域子系统，粒子彼此可以分辨。
        \item 非定位系统 non-localized system：又称离域子系统，基本粒子之间不可区分。
        \item 在总粒子数一定情况下，定位系统的微观状态数要比非定位系统的多得多。
    \end{enumerate}
    \item 根据统计单位之间有无相互作用
    \begin{enumerate}
        \item （近）独立粒子系统 assembly of independent particles：粒子之间相互作用忽略不计，系统总能量等于各个粒子能量之和。
        \begin{equation*}
            U = \sum_{i} N_i E_i
        \end{equation*}
        \item 非独立粒子系统：粒子之间的相互作用不能忽略，总能量中应包含粒子间相互作用的位能项。
        \begin{equation*}
...
\begin{enumerate}
    \item 热力学概率 $\Omega$：系统在一定的宏观状态下可能出现的微观态的总数，$S = k \ln{\Omega}$。
    \item 统计热力学中的一个基本假定（等概率假定）：对于 $U$, $V$ 和 $N$ 确定的某一宏观系统，任何一个可能出现的微观状态都有相同的数学概率。每一种微观状态 $P$ 出现的数学概率都相等，$\displaystyle P = \frac1{\Omega}$，所提供的微观量在平均值内的贡献都是一样的。
\end{enumerate}
...
一个由 $N$ 个可区分的独立粒子组成的宏观系统（$U$，$V$，$N$ 为定值），在量子化的能级上可以有多种不同的分配方式。
...
\noindent 无论哪一种分布方式，都满足以下条件：总粒子数守恒、总能量守恒，即
\begin{equation}
...
\end{equation}
\begin{equation}
    N^*_i = e^{\alpha + \beta \varepsilon_i} \label{MostProbableDistribution}
\end{equation}

\subsubsection[alpha, beta值的推导]{$\alpha$, $\beta$ 值的推导}

在 (\ref{MostProbableDistribution}) 中，
...
代入得
\begin{equation}
    N^*_i = N \frac{e^{-\varepsilon_i/k_{\mathrm{B}}T}}{\sum e^{-\varepsilon_i/k_{\mathrm{B}}T}}
\end{equation}
...
\begin{enumerate}
    \item 简并度
    \begin{equation}
        N^*_i = N \frac{g_i e^{-\varepsilon_i/k_{\mathrm{B}}T}}{\sum g_i e^{-\varepsilon_i/k_{\mathrm{B}}T}}
    \end{equation}
    ...
    \begin{enumerate}
        ...
        \begin{equation}
            N^*_i = N \frac{g_i e^{-\varepsilon_i/k_{\mathrm{B}}T}}{\sum g_i e^{-\varepsilon_i/k_{\mathrm{B}}T}}
        \end{equation}
        定位系统和非定位系统最概然分布的公式一致，但 $S$ 和 $A$ 表示式不相同。
        \end{enumerate}
\end{enumerate}
...
在经典统计中不考虑简并度，则上式可写为
\begin{equation}
    \frac{N^*_i}{N^*_j} = \frac{e^{-\varepsilon_i/k_{\mathrm{B}}T}}{e^{-\varepsilon_j/k_{\mathrm{B}}T}} = \exp\left(-\frac{\varepsilon_i - \varepsilon_j}{k_{\mathrm{B}}T}\right)
\end{equation}
...
\begin{enumerate}
    \item 令 $\sum g_i e^{-\varepsilon_i/k_{\mathrm{B}}T} \equiv q$，$q$ 称为离子的配分函数。
    \item 从上式可得
    \begin{equation}
        \frac{N_i}{N} = \frac{g_i e^{-\varepsilon_i/k_{\mathrm{B}}T}}{q}
    \end{equation}
    \item $q$ 中的任一项 $g_i e^{-\varepsilon_i/k_{\mathrm{B}}T}$ 与 $q$ 之比，等于粒子分配在 $i$ 能级上的分数。
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{equation}
        U = A + TS = ... = N k_{\mathrm{B}} T^2
    \end{equation}
    \item Gibbs 自由能
    
    根据定义 $G = A + pV$，将 $A$，$p$ 值代入，得
    \begin{equation}
        G_{\text{非定位}} = -k_{\mathrm{B}} T \ln{\frac{q^N}{N!}} + N
    \end{equation}
    \item 焓
    \begin{equation}
        H = G + TS = N k_{\mathrm{B}} T V
    \end{equation}
    \item 等容热容 $C_V$
    \begin{equation}
        C_V = \left(\frac{\partial U}{\partial T}\right)_V = 
    \end{equation}
    \item 从以上的表达式可以看出，只要知道配分函数 $q$ 就能求出诸热力学函数。
    \item 对于定位系统，用同样的方法也可以导出其热力学函数
    \begin{equation}
        A_{\text{定位}} = -k_{\mathrm{B}} T \ln{q^N}
    \end{equation}
    \item 由上述公式可见无论是定位系统或非定位系统 $U, H, C_V$ 的表示式是一样的，只是在热力学函数 $A, S, G$ 上相差一些常数项。
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{equation*}
        \begin{aligned}
            \sum_{i} g_i \exp\left(-\frac{\varepsilon_i}{k_{\mathrm{B}} T}\right) \quad \text{平动}
        \end{aligned}
    \end{equation*}
    \begin{equation}
...
\begin{enumerate}
    ...
    \begin{equation*}
        \begin{aligned}
            ...
            &= g_{\mathrm{n},0} \exp\left(-\frac{\varepsilon_{\mathrm{n},0}}{k_{\mathrm{B}} T}\right) \\
        \end{aligned}
    \end{equation*}
    ...
    \end{equation}
    式中 $s_{\mathrm{n}}$ 为核自旋量子数。
    \item 对于多原子分子，核的总配分函数等于各原子的核配分函数的乘积（不是加和），即
    \item 由于核自旋配分函数与温度、体积无关，所以对热力学能、焓和热容没有贡献，但在熵、 Helmholtz 自由能、 Gibbs 自由能中有相应的贡献。
\end{enumerate}
...
\begin{enumerate}
    ...
    \end{equation}
    \item 当能量为 $\varepsilon_i$ 时，由于 $n_x, n_y, n_z$ 不同而有不同的微观状态数，因此式 (7.80) 中的 $g_{i,\mathrm{t}}$ 是该能级的简并度。在式 (7.84) 中是对所有的 $n_x, n_y, n_z$ 求和，它巳经包括了全部可能的微观状态，因此就不再出现 $g_{i,\mathrm{t}}$ 项了。
    \item 平动配分函数在不同轴上的可分离性
    \item 式 (7.84) 由 完全相似的三项组成，只需解其中的一个，其余的可以类推。
